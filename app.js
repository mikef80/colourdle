...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let kbDis = false;
  let guessCountArr =
    JSON.parse(window.localStorage.getItem("guessCountArray")) || [];
  let gameDurationArr =
    JSON.parse(window.localStorage.getItem("gameDurationArray")) || [];
  let boardArr = [];
  let sharePic;
  let startTime;
  let finishTime;
  ...
  statsShare.addEventListener("click", () => {
    ...
    let dayCount =
      Math.floor((date.getTime() - launchDate.getTime()) / 86400000) + 1;
  ...
  function setSwatch() {
    let today = new Date();
    let yearBegin = new Date(2024, 2, 13); //also changed this value to match date above.  was 2022,0,15
  ...
  function handleSubmitGuess() {
    ...
    if (!checkInputs(currentGuessArr)) {
      // console.log('Error');
      window.alert(
        "Each seperate RGB value must be between 0 and 255. Have another go!"
      );
      return;
    } /* else {
      console.log('fine');
    ...
    if (currentGuess === answer) {
      ...
      if (
        today !== lastPlayedDate ||
        (today === lastPlayedDate && status !== "WON")
      ) {
        // ADD MAX STREAK
        finishTime = new Date();
        ...
        window.localStorage.setItem(
          "gameDurationArray",
          JSON.stringify(gameDurationArr)
        );
    ...
    if (guessedRGB.length === rows) {
      ...
      if (
        today !== lastPlayedDate ||
        (today === lastPlayedDate && status !== "LOST")
      ) {
        finishTime = new Date();
        gameDuration = (finishTime - startTime) / 1000;
        gameDurationArr.push(gameDuration);
        ...
        setTimeout(() => {
          window.alert(
            `Sorry, you have no more guesses! The answer is ${outputAnswer}`
          );
          toggleStats();
          createShareGrid(boardArr);
        }, interval * 9);
  ...
  function updateStats() {
    ...
    let currentStreakLS = Number(
      window.localStorage.getItem("currentStreak") || 0
    );
    let maxStreakLS = Number(window.localStorage.getItem("maxStreak") || 0);

    let maxStreakVar =
      currentStreakLS > maxStreakLS ? currentStreakLS : maxStreakLS;
